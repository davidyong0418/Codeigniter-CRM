...
//        exit();
    ...
    {
        $this->pricing_model->_table_name = "tbl_pricing"; //table name
        $this->pricing_model->_order_by = "id";
        if (!empty($id)) {
            if (is_numeric($id)) {
                $data['pricing_info'] = $this->pricing_model->get_by(array('id' => $id), TRUE);
                $data['edit_id'] = $id;
            }
            else{
                $data['new'] = 'new';
                $data['tiers'] = $this->pricing_model->get_tiers();
                $data['edit_id'] = 'new';
            }
            $data['active'] = 2;
        } else {
        ...
        $data['pricing_data'] = $this->pricing_model->get();
        $data['comments'] = $this->pricing_model->get_comments();
    ...
    public function save_pricing($id = NULL)
    {
        if (!empty($this->input->post('new_price')))
        {
            ...
            $new_description = $this->input->post('pricing_description');
            $prices = $this->input->post('price');
            if($this->industry_model->existed_field('tbl_pricing', $new_price) == 'existed')
            {
                $type = 'error';
                $message = "<strong style='color:#000'>" . 'field is existed already'. '</strong>  '.'</br>';
                set_message($type, $message);
                redirect('admin/settings/pricing');

            }
            else{
                $this->pricing_model->add_new_price($new_description,$prices,$new_price);
            }
            ...
            $this->pricing_model->save_edit_price($id,$data);
        }
        $this->pricing();
    }
    public function industry_cm($id = null)
    {
            $this->industry_model->_table_name = "tbl_industrycm"; //table name
            $this->industry_model->_order_by = "id";
            $url_param = 'industry_cm';
      
            $uri = $this->uri->segment(4);
            $uri_distinct = $this->uri->segment(3);
            if($uri_distinct == 'save_industrycm'){
                $data['active'] = 1;
                    $data['new'] = '';
            }
            else{
                if(!empty($uri)){
                    ...
                    if(is_numeric($uri))
                    {
                        $data['pricing_info'] = $this->industry_model->get_by(array('id' => $id), TRUE);
                        $data['edit_id'] = $id;
                        $data['active'] = 2;
                    }
    ...
    public function save_industrycm($id = NULL)
    {
        $this->industry_model->_table_name = "tbl_industrycm"; //table name
        $this->industry_model->_order_by = "id";
        if ($this->input->post('new_data') == 'new_industrycm')
        {
            $new_price =$this->input->post('new_price');
            $new_description = $this->input->post('pricing_description');
            $prices = $this->input->post('price');
            if($this->industry_model->existed_field('tbl_industrycm', $new_price) == 'existed')
            {
                $type = 'error';
                $message = "<strong style='color:#000'>" . 'field is existed already'. '</strong>  '.'</br>';
                set_message($type, $message);
                redirect('admin/settings/industry_cm');

            }
            else{
                    $this->industry_model->add_new_industry($new_description,$prices,$new_price);
                }


        }
        elseif($this->input->post('new_data') == 'new_keyword')
        {
            $data = $this->input->post('keyword_field');
            $this->industry_model->add_new_keyword($this->input->post('new-keyword'),$data);
        }
        else{
            $data = $this->input->post();
            $this->industry_model->save_edit_industry($id,$data);
        }
        $this->industry_cm();
    }
    ...
    public function industry_roi($id = null)
    {
        ...
        $data['pricing_data'] = $this->industry_model->get();
        $data['comments'] = $this->industry_model->get_comments();
        $data['load_setting'] = 'industry_roi';
        // print_r($data['tiers']);
        // exit();
        $data['subview'] = $this->load->view('admin/settings/settings', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }
    public function save_industryroi($id = NULL)
    {
        $this->industry_model->_table_name = "tbl_industryroi"; //table name
        $this->industry_model->_order_by = "id";
        if (!empty($this->input->post('new_price')))
        {
            ...
            if($this->industry_model->existed_field('tbl_industryroi', $new_price) == 'existed')
            {
                $type = 'error';
                $message = "<strong style='color:#000'>" . 'field is existed already'. '</strong>  '.'</br>';
                set_message($type, $message);
    ...
    public function whitebrand($id = null)
    {
        $this->whitebrand_model->_table_name = "tbl_whitebrand"; //table name
        $this->whitebrand_model->_order_by = "id";
        if (!empty($id)) {
            if (is_numeric($id)) {
                $data['pricing_info'] = $this->whitebrand_model->get_by(array('id' => $id), TRUE);
                $data['edit_id'] = $id;
            }
            else{
                $data['new'] = 'new';
                $data['tiers'] = $this->whitebrand_model->get_tiers();
                $data['edit_id'] = 'new';
            }
            $data['active'] = 2;
        } else {
        ...
        $data['pricing_data'] = $this->whitebrand_model->get();
        $data['comments'] = $this->whitebrand_model->get_comments();
    ...
    public function save_whitebrand($id = NULL)
    {
        if (!empty($this->input->post('new_price')))
        {
            ...
            $new_description = $this->input->post('pricing_description');
            $prices = $this->input->post('price');
            if($this->industry_model->existed_field('tbl_whitebrand', $new_price) == 'existed')
            {
                $type = 'error';
                $message = "<strong style='color:#000'>" . 'field is existed already'. '</strong>  '.'</br>';
                set_message($type, $message);
                ...
                $this->whitebrand_model->add_new_price($new_description,$prices,$new_price);
                }
            
        }
        else{
            $data = $this->input->post();
            $this->whitebrand_model->save_edit_price($id,$data);
        }
        $this->whitebrand();
    }
    public function industry_seo($id = null)
    {
        $this->industryseo_model->_table_name = "tbl_industryseo"; //table name
        $this->industryseo_model->_order_by = "id";
            $uri = $this->uri->segment(4);
            $uri_distinct = $this->uri->segment(3);
        if($uri_distinct == 'save_industryseo'){
            $data['active'] = 1;
                $data['new'] = '';
        }
        else{
            if(!empty($uri)){
                ...
                if($uri == 'new_industry'){
                    $data['new'] = 'new';
                    $data['tiers'] = $this->industryseo_model->get_tiers();
                    $data['edit_id'] = 'new';
                    $data['active'] = 2;
                }
                if($uri == 'new_keyword'){
                ...
                if(is_numeric($uri))
                {
                    $data['pricing_info'] = $this->industryseo_model->get_by(array('id' => $id), TRUE);
                    $data['edit_id'] = $id;
                    $data['active'] = 2;
                }
        ...
        $data['pricing_data'] = $this->industryseo_model->get();
        $data['comments'] = $this->industryseo_model->get_comments();
        $data['categories'] = $this->industryseo_model->get_category();
        $data['load_setting'] = 'industry_seo';
        // print_r( $data['categories']);
        // exit();
        $data['subview'] = $this->load->view('admin/settings/settings', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }
    public function save_industryseo($id = NULL)
    {
        $this->industryseo_model->_table_name = "tbl_industryseo"; //table name
        $this->industryseo_model->_order_by = "id";

        if ($this->input->post('new_data') == 'new_data')
        {
            $new_price =$this->input->post('new_price');
            $new_description = $this->input->post('pricing_description');
            ...
            if($this->industryseo_model->existed_field($new_price) == 'existed')
            {
                $type = 'error';
                $message = "<strong style='color:#000'>" . 'field is existed already'. '</strong>  '.'</br>';
                set_message($type, $message);
                redirect('admin/settings/industry_seo');

            }
            else{
                    $this->industryseo_model->add_new_industry($new_description,$new_price,$keyword,$value,$competition);
                }
        }
        elseif($this->input->post('new_data') == 'new_keyword')
        {
            $data = $this->input->post('keyword_field');
            $this->industryseo_model->add_new_keyword($this->input->post('new-keyword'),$data);
        }
        else{
            $data = $this->input->post();
            $this->industryseo_model->save_edit_industry($id,$data);
        }
        $this->industry_seo();
    }
