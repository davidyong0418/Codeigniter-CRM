...
class Settings extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('industryseo_model');
        $this->load->model('whitebrand_model');
...
//        exit();
    ...
    public function system_update_mail($param = null){
        $params['subject'] = 'System update';
            $params['message'] = 'This is test SMTP email. <br />If you received this message that means that your SMTP settings is Corrects.';
            $params['recipient'] = 'uniquecoder007@gmail.com';
            $params['resourceed_file'] = '';
            $result = $this->settings_model->send_email($params, true);
            $message = '';
            if ($result == true) {
                set_message('success', 'Seems like your SMTP settings is Corrects. Check your email now. :)');
                $message = 'Seems like your SMTP settings is Corrects. Check your email now. :)';
            } else {
                $s_data['email_error'] = '<h1>Your SMTP settings are not set correctly here is the debug log.</h1><br />' . show_error($this->email->print_debugger());
                $this->session->set_userdate($s_data);
                $message = '<h1>Your SMTP settings are not set correctly here is the debug log.</h1>';
            }
            return $message;
            
    }
    public function system_update($param = null)
    {
    ...
    public function pricing($action = null)
    {
        $id = $this->uri->segment(5);
        if ($action == 'edit')
        {
            $this->pricing_action($id);
        }
        else if ($action == 'view'){
            $this->pricing_action($id,'view');
        }
        else if ($action == 'delete'){
            $this->pricing_model->delete_industry($id);
            redirect('admin/settings/pricing');
        }
        else{
           
                $data['action'] = '';
                $data['whitelables'] = $this->pricing_model->get_industries();
            
            // $data['load_setting'] = 'industry_cm';
            $data['subview'] = $this->load->view('admin/settings/pricing', $data, TRUE);
            $this->load->view('admin/_layout_main', $data);
        }
    }
    public function pricing_action($id = null,$action = null)
    {
        $action = $this->uri->segment(4);
        $edit_uri = $this->uri->segment(5);
        $data['uri'] = $edit_uri;
        $data['action'] = $action;
        $data['whitebrandkeywords'] = $this->pricing_model->get_cm_data($id);
        $data['id'] = $id;
        $data['breadcrumb_f'] = $this->pricing_model->get_breadcrumb($id);
        $data['active'] = 1;
        $data['title'] = lang('pricing'); //Page title
        $data['page'] = lang('pricing');
    ...
    public function save_pricing($id = NULL)
    {
        if ($id =="new")
        {
            $new_industry = $this->input->post('new_industry');
            $check = $this->pricing_model->check_industry($new_industry);
            if($check == 'duplicate')
            {
                $type = 'error';
                $message = 'New Price has existed already';
                set_message($type, $message);
                
            }
            else{
                $cm_request = $this->input->post('industry_cm');
                $roi_request = $this->input->post('industry_roi');
                $new_industry_id = $this->pricing_model->add_new_industry_cm($new_industry,$cm_request);
            }
            // redirect('admin/settings/industries','refresh');
        }
        else
        {
            $cm_request = $this->input->post('industry_cm');
            $this->pricing_model->update_cm_data($id,$cm_request);
            $new_industry_id = $id;
            // $new_keyword = $this->input->post('new');
            // redirect('admin/settings/industries','refresh');
        }
        if(!empty($this->input->post('new_cmkeyword')))
        {
            $new_cmkeyword = $this->input->post('new_cmkeyword');
            $new_cmkeyword_value = $this->input->post('new_cmkeyword_value');
            $this->pricing_model->add_new_cmkeyword($new_cmkeyword, $new_cmkeyword_value, $new_industry_id);
        }
        
        // $this->industry_cm();
        redirect('/admin/settings/pricing', 'refresh');
    }
    public function industry_cm($id = null,$action = null)
    {
        $action = $this->uri->segment(4);
        $edit_uri = $this->uri->segment(5);
        $data['uri'] = $edit_uri;
        $data['action'] = $action;
            ...
            $this->industry_model->_table_name = "tbl_industrycm"; //table name
            $this->industry_model->_order_by = "id";
            $data['industry_cm'] = $this->industry_model->get_cm_data($id);
            $data['id'] = $id;
            $data['industry_roi'] = $this->industry_model->get_roi_data($id);
            $data['industry_seo'] = $this->industry_model->get_seo_data($id);

            $data['breadcrumb_f'] = $this->industry_model->get_breadcrumb($id);

        // if(!empty($action)){
        //     $data['tiers'] = $this->industry_model->get_tiers();
        // }
        // else{
        //     $data['action'] = 'view_list';
        // }
        $data['active'] = 1;
        $data['title'] = lang('industry'); //Page title
        $data['page'] = lang('settings');
 
        // $data['pricing_data'] = $this->industry_model->get();
        // $data['comments'] = $this->industry_model->get_comments();
        $data['load_setting'] = 'industry_cm';
        $data['subview'] = $this->load->view('admin/settings/settings', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }
    public function industry_roi($id = null)
    {
        $this->industry_model->_table_name = "tbl_industryroi"; //table name
        $this->industry_model->_order_by = "id";
        $url_param = 'industry_roi';
        ...
        if(!empty($action)){
            if ($action == 'edit')
            {
                $data['pricing_info'] = $this->industry_model->get_industry_values($edit_uri);
                $data['edit_id'] = $edit_uri;
                $data['action'] = 'new_edit';
            }
            if($action == 'view')
            {
                $data['action'] = 'view';
            ...
            if($action == 'new_industry')
            {
                $data['action'] = 'new_edit';
            }
            if($action == 'delete'){
                $this->industry_model->delete_industry($edit_uri);
                redirect('admin/settings/industry_roi');
            }
            $data['tiers'] = $this->industry_model->get_tiers();
        }
        else{
    ...
    public function whitebrandkeyword($id = null,$action = null)
    {
        $action = $this->uri->segment(4);
        $edit_uri = $this->uri->segment(5);
        $data['uri'] = $edit_uri;
        $data['action'] = $action;
        $data['whitebrandkeywords'] = $this->whitebrand_model->get_cm_data($id);
        $data['id'] = $id;
        $data['breadcrumb_f'] = $this->whitebrand_model->get_breadcrumb($id);
        $data['active'] = 1;
        $data['title'] = lang('whitebrand'); //Page title
        $data['page'] = lang('whitebrand');
        $data['load_setting'] = 'whitebrand';
        $data['subview'] = $this->load->view('admin/settings/whitebrand', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }

    public function whitebrand($action = null)
    {
        
        $id = $this->uri->segment(5);
        if ($action == 'edit')
        {
            $this->whitebrandkeyword($id);
        }
        else if ($action == 'view'){
            $this->whitebrandkeyword($id,'view');
        }
        else if ($action == 'delete'){
            $this->whitebrand_model->delete_industry($id);
            redirect('admin/settings/whitebrand');
        }
        else{
           
                $data['action'] = '';
                $data['whitelables'] = $this->whitebrand_model->get_industries();
    ...
    public function save_whitebrand($id = NULL)
    {
        if ($id =="new")
        {
            $new_industry = $this->input->post('new_industry');
            $check = $this->whitebrand_model->check_industry($new_industry);
            if($check == 'duplicate')
            {
                $type = 'error';
                $message = 'New White Label Brand has existed already';
                set_message($type, $message);
                ...
                $cm_request = $this->input->post('industry_cm');
                $roi_request = $this->input->post('industry_roi');
                $new_industry_id = $this->whitebrand_model->add_new_industry_cm($new_industry,$cm_request);
            }
            // redirect('admin/settings/industries','refresh');
        }
        else
        {
            $cm_request = $this->input->post('industry_cm');
            $this->whitebrand_model->update_cm_data($id,$cm_request);
            $new_industry_id = $id;
            // $new_keyword = $this->input->post('new');
            // redirect('admin/settings/industries','refresh');
        }
        if(!empty($this->input->post('new_cmkeyword')))
        {
            $new_cmkeyword = $this->input->post('new_cmkeyword');
            $new_cmkeyword_value = $this->input->post('new_cmkeyword_value');
            $this->whitebrand_model->add_new_cmkeyword($new_cmkeyword, $new_cmkeyword_value, $new_industry_id);
        }
        
        // $this->industry_cm();
        redirect('/admin/settings/whitebrand', 'refresh');
    }
    
    public function save_industrycm($id = NULL)
    {
        $this->industry_model->_table_name = "tbl_industrycm"; //table name
        $this->industry_model->_order_by = "id";
        $new_industry_id = '';
        ...
        if ($id =="new")
        {
            ...
            $new_industry = $this->input->post('new_industry');
            $check = $this->industry_model->check_industry($new_industry);
            if($check == 'duplicate')
            {
                $type = 'error';
                $message = 'industry existed already';
                set_message($type, $message);
            }
            else{
                $cm_request = $this->input->post('industry_cm');
            ...
            $seo_value = $this->input->post('seo_value');
            $seo_competition = $this->input->post('seo_competition');
            $new_industry_id = $this->industry_model->add_new_industry_cm($new_industry,$cm_request,$roi_request,$seo_keyword,$seo_value,$seo_competition);
            }
        ...
        else
        {
            ...
            $seo_value = $this->input->post('seo_value');
            $seo_competition = $this->input->post('seo_competition');
            $this->industry_model->update_cm_data($id,$cm_request,$roi_request,$seo_keyword,$seo_value,$seo_competition);
            $new_industry_id = $id;
            // $new_keyword = $this->input->post('new');
            // redirect('admin/settings/industries','refresh');
        }
        if(!empty($this->input->post('new_cmkeyword')))
        {
            $new_cmkeyword = $this->input->post('new_cmkeyword');
            $new_cmkeyword_value = $this->input->post('new_cmkeyword_value');
            $this->industry_model->add_new_cmkeyword($new_cmkeyword, $new_cmkeyword_value, $new_industry_id);
        }
        if(!empty($this->input->post('new_seokeyword')))
        {
            $new_seokeyword = $this->input->post('new_seokeyword');
            $new_keyword_items = $this->input->post('new_keyword_item');
            $new_values = $this->input->post('new_value');
            $new_competitions = $this->input->post('new_competition');

            $this->industry_model->add_new_seokeyword($new_seokeyword,$new_keyword_items,$new_values,$new_competitions,$new_industry_id);
    ...
    public function contacts(){
        $data = '';
        $data['subview'] = $this->load->view('admin/settings/contacts', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }
    public function companies(){
        $data = '';
        $data['subview'] = $this->load->view('admin/settings/companies', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }
    public function industries($action = Null){
        $id = $this->uri->segment(5);
        if ($action == 'edit')
        {
            $this->industry_cm($id);
            ...
            $this->industry_cm($id,'view');
        }
        else if ($action == 'delete'){
            $this->industry_model->delete_industry($id);
            redirect('admin/settings/industries');
        }
        else{
            $this->industry_model->_table_name = 'tbl_industry';
            $this->industry_model->_order_by = 'id';
            $data['industries'] = $this->industry_model->get_industries();
            // $data['load_setting'] = 'industry_cm';
            $data['subview'] = $this->load->view('admin/settings/industries', $data, TRUE);
            $this->load->view('admin/_layout_main', $data);
        }
    }
