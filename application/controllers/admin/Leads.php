...
class Leads extends Admin_Controller
{
    ...
    public function manage_lead($id = NULL)
    {
        if (!empty($id)) {
            if (is_numeric($id)) {
                $data['active'] = 2;
        ...
        if (!empty($id)) {
            ...
            $where = array('contact_type' => 1);
            $data['all_client_info'] = $this->db->where($where)->get('tbl_contact')->result();
        }
        $data['subview'] = $this->load->view('admin/leads/manage_lead', $data, TRUE);
    ...
    public function add_company_modal(){
        ...
        $data['subview'] = $this->load->view('admin/leads/add_company_modal', $data, FALSE);
    ...
    public function add_cold_call_type_modal(){
        $data['title'] = 'Add cold call type';
        $data['subview'] = $this->load->view('admin/leads/cold_call_type_modal', $data, FALSE);
        $this->load->view('admin/_layout_modal', $data);
    }
    ...
    public function add_additional_label_modal(){
        ...
        $data['subview'] = $this->load->view('admin/leads/additional_modal', $data, FALSE);
        $this->load->view('admin/_layout_modal', $data);
    }
    ...
    public function save_client($id = NULL)
    {
        ...
        if (!empty($_FILES['company_logo']['name'])) {
            ...
            $val == TRUE || redirect('admin/leads/manage_lead');
...
// sales
        ...
        if (!empty($save_and_create_contact)) {
            redirect('admin/leads/client_details/' . $id . '/add_contacts');
        } else {
            redirect('admin/leads/manage_lead');
        }
    ...
    public function contact_lead($id = null)
    {
        if (!empty($id)) {
            if (is_numeric($id)) {
                $data['active'] = 2;
...
//                }
    ...
    public function delete_lead($client_id, $id)
    {
        ...
        if (!empty($sbtn)) {
            ...
            // messages for user
            $type = "success";
            $message = 'Lead Information Successfully Deleted';
            set_message($type, $message);
            redirect('admin/lead/client_details/' . $client_id);
        } else {
    ...
    public
    function delete_leads($client_id, $yes = null)
    {
        $sbtn = $this->input->post('submit', true);
        if (!empty($sbtn) && !empty($yes)) {
            ...
            $data['subview'] = $this->load->view('admin/leads/delete_lead', $data, TRUE);
            $this->load->view('admin/_layout_main', $data); //page load
        }
    }
    ...
    public function lead_details($id = null)
    {
        ...
        $data['subview'] = $this->load->view('admin/leads/lead_details', $data, TRUE);
        $this->load->view('admin/_layout_main', $data); //page load
    }
}
