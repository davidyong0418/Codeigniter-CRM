...
class Login_Model extends MY_Model
{
    ...
    public function login()
    {
        ...
        $this->_table_name = 'tbl_contact';
        $this->_order_by = 'id';

        $contact_admin = $this->get_by(array(
            'nick_name' => $this->input->post('user_name'),
            'password' => $this->hash($this->input->post('password')),
        ), TRUE);
        if (!empty($contact_admin)) {
            $url = $this->session->userdata('url');
            $fullname = $contact_admin->first_name." ".$contact_admin->last_name;
            $data = array(
                'user_name' => $contact_admin->nick_name,
                'email' => $client_or_lead->email,
                'name' => $fullname,
                'photo' => 'uploads/business_card.jpg',
                'designations_id' => '',
                'user_id' => $contact_admin->id,
                'last_login' => NULL,
                'online_time' => time(),
                'loggedin' => TRUE,
                'user_type' => $contact_admin->contact_type,
                'user_flag' => 1,
                'direction' => '',
                'url' => (!empty($url) ? $url : 'admin/dashboard'),
            );
